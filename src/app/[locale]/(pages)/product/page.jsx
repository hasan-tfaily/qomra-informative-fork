...
import ProductReviewItem from "@/src/app/_components/products/ProductReviewItem";
import ProductButtonsSection from "@/src/app/_components/products/ProductButtons";
import ShareButtonsSection from "@/src/app/_components/ShareButtons";
import ProductRelatedSection from "@/src/app/_components/products/ProductRelated";

const ProductImagesSlider = dynamic(
  () => import("@/src/app/_components/sliders/ProductImages"),
  { ssr: true }
);
const ProductTabs = dynamic(
  () => import("@/src/app/_components/products/ProductTabs"),
  { ssr: true }
);
  ...
  async function ProductDescription() {
    return (
      <>
        ...
        <p className="mil-text mil-soft mil-up">
          Fringilla phasellus, placerat auctor litora consectetur praesent netus
          aptent ut ornare netus dictum vivamus et volutpat, nunc lorem enim
          urna pellentesque egestas gittis ut magna consequat nibh turpis.
        </p>
      </>
    );
  }
        ...
        <div className="container">
          <div className="row mil-jcc">
            <div className="col-md-8">
              ...
              <p className="mil-fs30 mil-soft mil-mb60 mil-up">
                {ProductData.currency}
                {ProductData.price}
              </p>
              <p className="mil-text mil-soft mil-mb60 mil-up">
                {ProductData.short}
              </p>
              ...
              <ProductTabs items={tabs} active={"description"} />

              {tabs.map((tab, key) => (
                <div
                  id={`tab-${tab.slug}`}
                  className={
                    key == 0 ? "mil-tab-content mil-current" : "mil-tab-content"
                  }
                  key={`product-tab-${key}`}
                >
